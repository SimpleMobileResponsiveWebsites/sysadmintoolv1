...
# Initialize session state for tasks and subtasks
...
with st.sidebar.form("task_form"):
    ...
    task_category = st.selectbox(
        "Category",
        [
            "System Maintenance & Support", "Network Management", "Security & Compliance", 
            "User Support & Training", "Backup & Disaster Recovery", 
            "Project Management & Implementation", "Other IT-Related Duties"
        ]
    )
    ...
    if st.form_submit_button("Add Task"):
        if task_title and task_assigned_to:
            new_task = {
                ...
                "Due Date": task_due_date, "Assigned To": task_assigned_to, 
...
if st.session_state.tasks:
    ...
    if not tasks_df.empty:
        updated_tasks_df = st.data_editor(
            tasks_df, use_container_width=True, hide_index=True, key="editable_task_table"
        )
